...
```{r}
cellphonedb = read.csv("../Spreadsheets/significant_means.csv",stringsAsFactors = F)
...
Load in barres data, use it to remove genes from sobject that are specific to neurons
```{r}
sobject = mgAVM02
barres = read.csv("../Spreadsheets/barreslab_rnaseq.csv",stringsAsFactors = F)
...
```{r}
meta = paste0("microglia_",sobject1$finalclusters)
names(meta) = colnames(sobject1)
meta2 = sobject2$celltypecluster
meta = as.data.frame(c(meta,meta2))
...
countsm = countsm[rownames(countsm) %in% m,]
...
Cellphone db: also from jupyter, see python script for details. "X3.04" means from cluster 3 (source) to cluster 4 (target)
```{r}
cellphonedb = read.table("../Spreadsheets/OPCsignificant_means.txt",stringsAsFactors = F)
#cellphonedb = read.csv("https://github.com/lcdorman/scrnaseq/blob/master/significant_means.csv",stringsAsFactors = F)
colnames(cellphonedb)
```
count per interaction, remove rows with nothing
