...
geneExpr = as.matrix(geneExpr[rownames(geneExpr)%in% all.genes,])
geneExpr = geneExpr[rowSums(geneExpr)>100,]
geneCounts = as.matrix(geneCounts[rownames(geneCounts)%in% all.genes,])
geneCounts = geneCounts[rowSums(geneCounts)>1000,]


info = sobject@meta.data
form <- ~ percent.mito + percent.ribo
#form <- ~ percent.mito + (1|sex) + (1|age) + (1|condition)

varPart <- fitExtractVarPartModel(geneCounts, form, info )
# sort variables (i.e. columns) by median fraction of variance explained
vp <- sortCols(varPart )

#order on sex
vs = varPart[order(varPart$percent.ribo,decreasing = T),]
vm = varPart[order(varPart$percent.mito,decreasing = T),]

# Bar plot of variance fractions for the first 10 genes
plotPercentBars( vm[1:50,] )
