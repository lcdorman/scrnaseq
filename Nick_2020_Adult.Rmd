...
save(sobject,file = "~/Desktop/mpeg_adult.RData")
```
...
for(feature in c(genes,features)){
  PrintSeuratGraph(namecard = "Adult_mpeg",sobject=sobject,graphtype = "feature",feature = feature)
}

for(feature in c(genes,features)){
  PrintSeuratGraph(namecard = "Adult_mpeg_split",sobject=sobject,graphtype = "feature",feature = feature,split = "individual")
}

groups = c("sample_description","seurat_clusters")

for(group in groups){
  PrintSeuratGraph(namecard = "Adult_mpeg_individual",sobject=sobject,graphtype = "dim",group = group, feature = group,split = "individual")
}

for(group in groups){
  PrintSeuratGraph(namecard = "Adult_mpeg",sobject=sobject,graphtype = "dim",group = group, feature = group)
}

genes = c("apoeb", "p2ry12", "hexb", "csf1rb", "irf8", "slc7a7","mpeg1.1","ptprc")

for (gene in genes){
  PrintSeuratGraph(namecard = "Adult_mpeg_vln",sobject=sobject,graphtype = "violin",group = "seurat_clusters", feature = gene)
}
...
setEPS()
postscript("~/Desktop/tree_adult_mpeg.eps")
...
dim(markers_all)
head(markers_all)
write.csv(markers_all,file = "~/Desktop/adult_mpeg_markers.csv")
...
topgenes <- markers_all_single_thresh %>% group_by(cluster) %>% top_n(10, avg_logFC)

setEPS()
postscript("~/Desktop/heatmap_adultmpeg_pval50.eps")
DoHeatmap(
    object = sobject, 
