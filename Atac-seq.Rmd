...
cd
```
Make a table. You'll need to change the numbers if you have more or less than 4 clusters. This actually just repeats the sequence of 4 numbers all the way down. 
```{r}
class(cd)
cd = as.data.frame(cd)
dim(cd)
cd$three = cd[1:4,1] #source = three
cd$four = cd[5:8,1] #source = four
cd$zero = cd[9:12,1] #source = zero/five
cd$eight = cd[13:16,1] #source = eight
dim(cd)
```
Make a table of rows = target (3,4,5,8) (specify 4 rows) and columns = source (3,4,5,8) (remove column 1, it's not meaningful)
```{r}
cd = cd[1:4,2:5]
cd
```
...
setEPS()
postscript("~/Desktop/heatmap_cellphonedb.eps")
